...
_TAR_FILE = 'dist-p_blog.tar.gz'

_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE

_REMOTE_BASE_DIR = '/srv/p_blog'
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-p_blog-%s.sql' % dt
    with cd('/tmp'):
        run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick p_blog > %s' % (db_user, db_password, f))
...
def deploy():
    ...
    with settings(warn_only=True):
        sudo('supervisorctl stop p_blog')
        sudo('supervisorctl start p_blog')
...
def restore2local():
    ...
    sqls = [
        'drop database if exists p_blog;',
        'create database p_blog;',
        'grant select, insert, update, delete on p_blog.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
        local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
    with lcd(backup_dir):
        local('tar zxvf %s' % restore_file)
    local(r'mysql -uroot -p%s p_blog < backup/%s' % (p, restore_file[:-7]))
    with lcd(backup_dir):
